...
function labelStatus(localeKey: string, t: (k: string) => string): string {
  ...
  const STATS = [
    {
      ...
      change:
        total === 0
          ? t('dashboard.statTotalZero')
          : total === 1
            ? t('dashboard.statTotalOne')
            : t('dashboard.statTotalMany').replace('{{n}}', String(total)),
    },
    {
      ...
      change:
        inProgress === 0
          ? t('dashboard.statProgressZero')
          : inProgress === 1
            ? t('dashboard.statProgressOne')
            : t('dashboard.statProgressMany').replace('{{n}}', String(inProgress)),
    },
    {
      ...
      change:
          ...
          ? t('dashboard.statDoneZero')
          : done === 1
            ? t('dashboard.statDoneOne')
            : t('dashboard.statDoneMany').replace('{{n}}', String(done)),
    },
    {
      ...
      change: t('dashboard.avgProgressHint'),
    },
  ]
      ...
      <div className="flex-1 p-4 md:p-6 space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold text-[#111827]">{t('dashboard.recentTitle')}</h2>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <Badge className={getStatusColor(project.status)}>
                          {labelStatus(project.status as ProjectStatus, t)}
                        </Badge>
                      </div>
                      <p className="text-sm text-[#6B7280] truncate">{project.description ?? t('common.emDash')}</p>
                    </div>
                    <div className="w-36 flex-shrink-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[#6B7280]">{t('common.progress')}</span>
