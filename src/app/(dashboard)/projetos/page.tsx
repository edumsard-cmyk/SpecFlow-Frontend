...
export default function ProjetosPage() {
  ...
  const counts = useMemo(() => {
    const c: Record<string, number> = { all: MOCK_PROJECTS.length }
  ...
  return (
    <div className="flex flex-col flex-1">
      <Header
        title="Projetos"
        subtitle={`${MOCK_PROJECTS.length} projetos no total`}
        actions={
      ...
      <div className="flex-1 p-6 space-y-5">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div className="flex items-center border border-[#E5E7EB] rounded-lg bg-white overflow-hidden">
              <button
                ...
                className={`p-2 transition-colors ${view === 'grid' ? 'bg-[#1E3A8A] text-white' : 'text-[#6B7280] hover:bg-[#F8FAFC]'}`}
              ...
              <button
                ...
                className={`p-2 transition-colors ${view === 'list' ? 'bg-[#1E3A8A] text-white' : 'text-[#6B7280] hover:bg-[#F8FAFC]'}`}
        ...
        {filtered.length > 0 && (
          <p className="text-xs text-[#9CA3AF] text-center pt-2">
            Exibindo {filtered.length} de {MOCK_PROJECTS.length} projetos
          </p>
        )}
      </div>
    </div>
